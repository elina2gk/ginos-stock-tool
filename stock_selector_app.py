...
if "refresh_choice" not in st.session_state:
    st.session_state.refresh_choice = "30 seconds"
refresh_map = {"Off": 0, "30 seconds": 30, "60 seconds": 60, "2 minutes": 120}
refresh_sec = refresh_map.get(st.session_state.refresh_choice, 0)
...
PAGES = ["Portfolio", "Discover", "Leaders", "Screener", "My Watchlist", "Alerts", "Earnings & IPOs"]
...
for i, p in enumerate(PAGES):
    if nav_cols[i].button(p, use_container_width=True):
        ...
        st.rerun()
nav_cols[-1].caption(datetime.now().strftime("%H:%M:%S"))
...
# ========================= MY WATCHLIST =========================
    ...
    picked = st.selectbox("Type or choose a symbol", all_choices, key="wl_pick")
    typed = st.text_input("Or type a ticker", key="wl_typed").strip().upper()
    chosen = typed or (picked.split()[0] if picked else "")
    add1, add2 = st.columns(2)
    add_stock = add1.button("Add to Stock watchlist")
    add_etf = add2.button("Add to ETF watchlist")
    if add_stock or add_etf:
        if not chosen:
            st.error("Choose or type a symbol first.")
        else:
            if add_stock:
                if chosen not in wl_cfg["watchlist_stocks"]:
                    wl_cfg["watchlist_stocks"].append(chosen)
                wl_cfg["watchlist_etfs"] = [x for x in wl_cfg["watchlist_etfs"] if x != chosen]
            else:
                if chosen not in wl_cfg["watchlist_etfs"]:
                    wl_cfg["watchlist_etfs"].append(chosen)
                wl_cfg["watchlist_stocks"] = [x for x in wl_cfg["watchlist_stocks"] if x != chosen]
    ...
    with s_col:
        ...
        if not wl_cfg["watchlist_stocks"]:
            st.caption("None yet")
        else:
            st.write(", ".join(wl_cfg["watchlist_stocks"]))
            rm_s = st.selectbox("Remove stock", [""] + wl_cfg["watchlist_stocks"], key="rm_stock")
            if st.button("Remove stock") and rm_s:
                wl_cfg["watchlist_stocks"] = [x for x in wl_cfg["watchlist_stocks"] if x != rm_s]
                save_alert_settings(wl_cfg)
                st.rerun()
    with e_col:
        st.markdown("**ETF watchlist**")
        if not wl_cfg["watchlist_etfs"]:
            st.caption("None yet")
        else:
            st.write(", ".join(wl_cfg["watchlist_etfs"]))
            rm_e = st.selectbox("Remove ETF", [""] + wl_cfg["watchlist_etfs"], key="rm_etf")
            if st.button("Remove ETF") and rm_e:
                wl_cfg["watchlist_etfs"] = [x for x in wl_cfg["watchlist_etfs"] if x != rm_e]
                save_alert_settings(wl_cfg)
                st.rerun()
    ...
    if st.button("Save alert email settings"):
        save_alert_settings(wl_cfg)
...
# ========================= DISCOVER =========================
    ...
    with c1:
        mode = st.selectbox("Universe", ["All (Stocks + ETFs)", "Stocks only", "ETFs only", "Custom tickers"], key="mode")
    with c2:
        period = st.selectbox("Period", ["6mo", "1y", "2y", "5y"], index=1, key="period")
    with c3:
        min_score = st.slider("Min score", 0, 80, 50, key="min_score")
    custom_input = st.text_input("Custom tickers", value=st.session_state.get("custom_input", "AAPL MSFT NVDA QQQ VOO SCHD TSM CRWV SKHY"), key="custom_input")
    ...
    if df_prev is not None and not getattr(df_prev, "empty", True):
        ...
        st.markdown("Click **Find strong stocks & ETFs**. Keep Refresh set to Off.")
    ...
    try:
        ...
        if earn.empty:
            ...
            st.download_button(
                ...
                data=earn.to_csv(index=False).encode("utf-8"),
...
with f1:
    st.selectbox("Refresh", ["Off", "30 seconds", "60 seconds", "2 minutes"], key="refresh_choice")
with f2:
