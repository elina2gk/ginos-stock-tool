...
# ========================= MY WATCHLIST =========================
    ...
    picked = st.selectbox("Type or choose a symbol", all_choices, index=0)
    chosen = picked.split()[0] if picked else ""
    add1, add2 = st.columns(2)
    if add1.button("Add to Stock watchlist") and chosen:
        if chosen not in wl_cfg["watchlist_stocks"]:
            wl_cfg["watchlist_stocks"].append(chosen)
        if chosen in wl_cfg["watchlist_etfs"]:
            wl_cfg["watchlist_etfs"].remove(chosen)
        save_alert_settings(wl_cfg)
        st.rerun()
    if add2.button("Add to ETF watchlist") and chosen:
        if chosen not in wl_cfg["watchlist_etfs"]:
            wl_cfg["watchlist_etfs"].append(chosen)
        if chosen in wl_cfg["watchlist_stocks"]:
            wl_cfg["watchlist_stocks"].remove(chosen)
        save_alert_settings(wl_cfg)
        st.rerun()
